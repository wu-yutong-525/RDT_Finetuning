...
class RoboticDiffusionTransformerModel(object):
    ...
    def __init__(
        ...
        print(f"ag_model device_1: {self.device_1}")
        print(f"ag_model device_2: {self.device_2}")
    ...
    def get_text_encoder(self, pretrained_text_encoder_name_or_path):
        text_embedder = T5Embedder(from_pretrained=pretrained_text_encoder_name_or_path, 
                                   ...
                                   device_2=self.device_2,use_offload_folder=True)
    ...
    def step(self, proprio, images, text_embeds):
        ...
        print(f"text_embeds size:{text_embeds.size()}")
        # Predict the next action chunk given the inputs
        # print(f"lang token device: {text_embeds.device}")
        image_embeds = image_embeds.to(device)
        # print(f"img tokens device: {image_embeds.device}")
        # print(f"state tokens device: {states.device}")
        # print(f"state_elem_mask device: {state_elem_mask.device}")
        # print(f"ctrl_freqs device: {ctrl_freqs.device}")
